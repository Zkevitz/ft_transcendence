...
exports.unregisterPlayerFromTournament = unregisterPlayerFromTournament;
...
exports.endTournamentMatch = endTournamentMatch;
...
async function createTournament(tournamentData) {
    return new Promise((resolve, reject) => {
      ...
      VALUES (?, ?, ?)
    `;
        database_1.db.run(query, [tournamentData.name, tournamentData.organizer_id, tournamentData.max_players], function (err) {
            if (err) {
                return reject(err);
            }
            resolve(this.lastID);
...
async function getTournamentById(tournamentId) {
    return new Promise((resolve, reject) => {
      ...
      WHERE id = ?
    `;
        database_1.db.get(query, [tournamentId], (err, row) => {
            if (err) {
                return reject(err);
            }
            resolve(row || null);
 ...
 */
async function getAllTournaments(status) {
    return new Promise((resolve, reject) => {
        let query = `
 ...
 */
async function registerPlayerToTournament(tournamentId, userId) {
    return new Promise(async (resolve, reject) => {
        ...
        VALUES (?, ?)
      `;
            database_1.db.run(query, [tournamentId, userId], function (err) {
                if (err) {
                    return reject(err);
                }
 ...
 * @returns Promise avec succès ou échec
 */
async function unregisterPlayerFromTournament(tournamentId, userId) {
    return new Promise(async (resolve, reject) => {
        try {
            // Vérification que le tournoi existe et est en phase d'inscription
            const tournament = await getTournamentById(tournamentId);
...
async function getTournamentParticipants(tournamentId) {
    ...
    `;
        database_1.db.all(query, [tournamentId], (err, rows) => {
            if (err) {
                return reject(err);
            }
            resolve(rows);
...
async function startTournament(tournamentId) {
    return new Promise(async (resolve, reject) => {
        ...
        WHERE id = ?
      `;
            database_1.db.run(updateQuery, [tournamentId], async function (err) {
                if (err) {
                    return reject(err);
                }
                if (this.changes === 0) {
 ...
 * @returns Promise avec succès ou échec
 */
async function generateTournamentMatches(tournamentId, participants) {
    return new Promise(async (resolve, reject) => {
        try {
          ...
          `;
                    ...
                    database_1.db.run(query, [tournamentId, match.round, match.position, match.player1_id, match.player2_id, status], function (err) {
                        if (err) {
                            return reject(err);
                        }
                        resolve();
 ...
 * @returns Promise avec succès ou échec
 */
async function updateNextRoundMatches(tournamentId) {
    return new Promise(async (resolve, reject) => {
        try {
      ...
      `;
            database_1.db.all(query, [tournamentId], async (err, completedMatches) => {
                if (err) {
                    return reject(err);
                }
              ...
              `;
                            database_1.db.run(updateQuery, [winnerId, tournamentId, nextRound, nextPosition], function (err) {
                                if (err) {
                                    return reject(err);
                                }
                                resolve();
...
async function startTournamentMatch(matchId) {
    return new Promise(async (resolve, reject) => {
        ...
        WHERE id = ?
      `;
            database_1.db.get(query, [matchId], async (err, match) => {
                if (err) {
                    return reject(err);
                }
                ...
                try {
                    ...
                    const gameId = await (0, gameService_1.createGame)({
            ...
            WHERE id = ?
          `;
                    database_1.db.run(updateQuery, [gameId, matchId], function (err) {
                        if (err) {
                            return reject(err);
                        }
                        resolve(gameId);
 ...
 * @returns Promise avec succès ou échec
 */
async function endTournamentMatch(matchId, winnerId) {
    return new Promise(async (resolve, reject) => {
        try {
            // Récupération du match
            const query = `
      ...
      `;
          ...
          WHERE id = ?
        `;
                database_1.db.run(updateQuery, [matchId], async function (err) {
                    if (err) {
                        return reject(err);
                    }
                    if (this.changes === 0) {
 ...
 * @returns Promise avec succès ou échec
 */
async function updateNextRoundWithWinner(tournamentId, round, position, winnerId) {
    return new Promise((resolve, reject) => {
        // Calcul de la position dans le tour suivant
...
async function checkTournamentCompletion(tournamentId) {
    return new Promise(async (resolve, reject) => {
        ...
        LIMIT 1
      `;
            database_1.db.get(query, [tournamentId], (err, finalMatch) => {
                if (err) {
                    return reject(err);
                }
          ...
          WHERE id = ?
        `;
                database_1.db.run(updateQuery, [tournamentId], function (err) {
                    if (err) {
                        return reject(err);
                    }
 ...
 * @returns Promise avec succès ou échec
 */
async function updatePlayerTournamentStats(tournamentId, winnerId) {
    return new Promise(async (resolve, reject) => {
        try {
            // Récupération des participants
            const participants = await getTournamentParticipants(tournamentId);
            ...
            WHERE user_id = ?
          `;
                    database_1.db.run(query, [participant.user_id, winnerId, participant.user_id], (err) => {
                        if (err) {
                            return reject(err);
                        }
                        resolve();
