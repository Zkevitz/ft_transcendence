...
interface Ball extends GameObject {
  dx: number;  // Direction horizontale (-1: gauche, 1: droite)
  dy: number;  // Direction verticale (-1: haut, 1: bas)
}
 ...
 */
function moveLeftPaddleUp(): void {
  if (!gameState) return;
 ...
 */
function moveLeftPaddleDown(): void {
  if (!gameState) return;
  gameState.leftPaddle.y += gameState.leftPaddle.speed;
 ...
 */
function moveRightPaddleUp(): void {
  if (!gameState) return;
 ...
 */
function moveRightPaddleDown(): void {
  if (!gameState) return;
  gameState.rightPaddle.y += gameState.rightPaddle.speed;
...
function drawGame(ctx: CanvasRenderingContext2D): void {
  ...
  ctx.fillText(
    gameState.leftPaddle.score.toString(),
    gameState.gameWidth / 4,
    50
  );
  ctx.fillText(
    gameState.rightPaddle.score.toString(),
    gameState.gameWidth * 3 / 4,
    50
  );
