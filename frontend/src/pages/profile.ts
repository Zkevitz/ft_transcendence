...
export async function renderProfilePage(container: HTMLElement): Promise<void> {
  ...
  try {
    ...
    ).slice(0, 5); // Prendre les 5 dernières parties
...
function showEditProfileModal(user: any): void {
  ...
  document.getElementById('edit-profile-form')?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      closeModal();
      renderProfilePage(document.getElementById('app-container') as HTMLElement);
 ...
 */
function showChangeAvatarModal(userId: number): void {
 ...
 */
function showGameHistoryModal(userId: number, games: any[]): void {
  // Implémentation à venir
